...
const createURIWithQueryParams = (protocol: Protocol, host: string, path: string, params: EngineControlParams) => {
...
export default class EngineApiService {
  ...
  setEngineDriveMode(carId: number): Promise<boolean> {
    ...
    return fetch(endpoint, { method: HTTPMethod.PATCH }).then((response) => response.ok);
  }
}
